...
def _enforce_airgap_from_env() -> None:
    """Install the outbound-network guard when ``SATQUERY_MODE`` is set.

    Doing this at package import covers child processes and CLI entry points
    (``python -m satquery.evaluate``, ``scripts/*``) without each one having to
    ...
    failure-tolerant so it can never break ``import satquery``.
    """
    import os
    if not os.environ.get("SATQUERY_MODE", "").strip():
        return
    try:

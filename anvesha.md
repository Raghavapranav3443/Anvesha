...
| **Captioner** | Writes scene descriptions via a small transformer decoder steered by a predicted content plan | BigEarthNet.txt captions joined to real Sentinel-2 patches | Multi-reference BLEU 0.283 | Deterministic template built from verified facts |
...
| **Change detector** | Compares two dates; twin networks + multi-scale difference decoder; tiled inference for big scenes | LEVIR-CD | Change IoU 0.60 / F1 0.75 | Smoothed image differencing |
...
| Change detection | F1 75 / IoU 60 | Published SOTA reaches F1 ~92 with far larger GPU-trained models; ours is the CPU-class capability demo. |
| Optical–SAR | 85% label recall | Strong for a 14k-pair subset trained on a laptop. |
| Captioning | BLEU 0.283 (multi-ref) | Protocol-dependent metric; single-ref training BLEU 0.59. |

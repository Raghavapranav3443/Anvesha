    ...
    <div className="space-y-2">
      {rules.map(({ rule, detail }, i) => (
        <div key={rule.when + i}
          ...
          <div className="text-sm font-medium text-warn">{rule.title}</div>
          <div className="mt-0.5 text-sm leading-snug text-body">
            {rule.text}{detail && (
              <span className="mt-1 block font-mono text-xs text-muted">{detail}</span>
            )}
          </div>
        </div>
      ))}

...
*(Anvesha = "search / investigation" in Sanskrit. Also known as Anvesha AI during development.)*
...
|---|---|---|---|---|
| **VQA** | Answers questions about one image; separate expert heads for yes/no, urban-vs-rural, comparisons, and counting | RSVQA-LR (54k image-question-answer triples) | 71% overall exact-match; per-type: presence **88%** (held-out val, n=5,414), rural/urban 84% | Rule-based reasoner |
| **Captioner** | Writes scene descriptions via a small transformer decoder steered by a predicted content plan | BigEarthNet.txt captions joined to real Sentinel-2 patches | Multi-reference BLEU 0.306 | Deterministic template built from verified facts |
...
| **Change detector** | Compares two dates; twin networks + multi-scale difference decoder; tiled inference for big scenes | LEVIR-CD | Change IoU 0.692 / F1 0.818 | Smoothed image differencing |
...
- **Provenance page.** Every number shown in the app comes from a measured checkpoint, re-runnable
  with one command: `python -m anvesha.evaluate --all`.- **Proven under pressure.** 386 automated tests pass offline (synthetic GeoTIFF fixtures), including
...
- **Batch mode:** `python -m anvesha.evaluate --sac-dir DIR` chews through a whole folder,
...
|---|---|---|
| Single-image VQA | **70.0% full test** (n=9,491) · 77.3% spot-check (n=282) | Below the original paper's 79% on the full test set; our per-type heads (presence 88% held-out val) are the stronger evidence. Known weakness, actively framed. |
...
| Captioning | BLEU 0.306 (multi-ref) | Protocol-dependent metric; single-ref training BLEU 0.59. |
...
# or fully offline:
docker build -t anvesha . && docker run -p 8000:8000 anvesha

python -m pytest tests -q            # 387 tests, offline-capable
ANVESHA_MODE=airgap python -m pytest tests -q     # same suite, network provably cut
...
python -m anvesha.evaluate --all    # regenerate spot-check numbers

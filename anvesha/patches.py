...
* ``enrich_result(result, images)`` attaches run-level additive keys
  ...
  ``report.json``.
* Kill-switch: ``ANVESHA_PATCHES=0`` disables the whole layer — tools run
...
def patches_enabled() -> bool:
    """False only when ANVESHA_PATCHES is explicitly "0"."""
...
def apply_patches(registry: Dict[str, Any]) -> None:
    ...
    for name, spec in registry.items():
        ...
        if fn is None or getattr(fn, "_anvesha_patched", False):
            continue                      # idempotent: never double-wrap
        wrapped = _wrap(name, fn)
        if wrapped is fn:
            continue
        wrapped._anvesha_patched = True
        spec.fn = wrapped

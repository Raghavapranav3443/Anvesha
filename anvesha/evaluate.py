"""Unified evaluation & SAC batch harness.
  ...
  python -m anvesha.evaluate --all                 # full public-benchmark card
  python -m anvesha.evaluate --sac-dir DIR         # batch ISRO/SAC-style pairs
"""
...
def bench_vrsbench_caption(n: int = 300) -> dict | None:
    ...
    from anvesha.models import describe
    items = _vrsbench_val()
    if not items:
        return None
    scores = []
...
def bench_vrsbench_grounding(n: int = 200) -> dict | None:
    ...
    """
    from anvesha.models import ground
    items = _vrsbench_val()
    if not items:
        return None
    ious = []
...
def bench_vrsbench_vqa(n: int = 300) -> dict | None:
    ...
    if not items:
        return None
    from anvesha.models import get_vqa_model
...
def sac_batch(folder: Path, out_csv: Path | None = None,
    ...
    out_dir = folder / "anvesha_outputs"
...
def main():
    ...
    CONFIG.artifact("scorecard.json").write_text(json.dumps(payload, indent=2))

    print("\n=== Anvesha AI scorecard ===")
    for r in card:

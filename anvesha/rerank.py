...
when ``ANVESHA_RERANK=0`` or the tower is unavailable — never raises.

CLIP contribution is **off by default** (``ANVESHA_RERANK_CLIP=1`` to enable).
...
def re_rank(query: str, ranked: List[Tuple[str, float]], top_k: int = 2,
    ...
    if clip_requested:
        try:
            ...
            # which resolves to ``anvesha.clip_text`` — a module that does not
            # exist (the real one is ``anvesha.models.clip_text``). The broad
